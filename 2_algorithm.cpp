 ...
 * Сейчас пока не могу понять, почему получилось так не очень:
 * может вообще некорректно применять этот метод к данной задаче, может я что-то не так сделала, а может
 * этот метод настолько неточный. Так вдобавок потом прочитала, что в задании нужно реализовать точно,
 * то есть, как я теперь уже поняла, перебором.
 */
